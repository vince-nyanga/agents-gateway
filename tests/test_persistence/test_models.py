...
async def test_execution_columns(db_engine: AsyncEngine):
    ...
    expected = {
        "id", "agent_id", "status", "message", "context", "options",
        "result", "error", "usage", "started_at", "completed_at", "created_at",
    }
...
async def test_schedules_columns(db_engine: AsyncEngine):
    ...
    async with db_engine.connect() as conn:
        columns = await conn.run_sync(
            lambda sync_conn: {
                c["name"] for c in inspect(sync_conn).get_columns("schedules")
            }
        )

    expected = {
        "id", "agent_id", "name", "cron_expr", "message", "context",
        "enabled", "timezone", "last_run_at", "next_run_at", "created_at",
    }
    assert expected.issubset(columns)
